...
class App extends Component {
  ...
  tick = () => {
    ...
    if (Timer === 0) {
      this.setState({GameOverCard: true, Timer: 0})
  ...
  selectedImage = id => {
    ...
    if (SingleImage.id === id) {
      this.setState(prevState => ({
        SingleImage: imagesList[Math.ceil(Math.random() * 10)],
        Score: prevState.count + 1,
      }))
    } else {
      this.setState({GameOverCard: true})
      clearInterval(this.timerID)
    }
  }
  ...
  onClickResetButton = () => {
    this.setState({GameOverCard: false, Score: 0, Timer: 60})
  }

  render() {
    const {activeTabId, SingleImage, Score, Timer, GameOverCard} = this.state

    const imagesResult = this.getFilteredList()
      ...
      <div className="main-container">
        ...
        {!GameOverCard && (
          <div className="image-container">
            <img src={SingleImage.imageUrl} alt="match" className="image2" />
            ...
            <ul className="listItem2">
              {tabsList.map(eachTab => (
                <TabItem
                  key={eachTab.tabId}
                  each={eachTab}
                  onClickingButton={this.clickTabItem}
